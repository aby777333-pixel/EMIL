...
export default function DeliveryPanel() {
  ...
  return (
    <Panel title="Alert Delivery — Telegram & Email" icon={Send} accent="amber" className="min-w-0 overflow-hidden">
      {!d ? <p className="text-xs text-slate-500">Loading…</p> : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2 gap-4 min-w-0">
          <div className="rounded-lg border border-border bg-secondary/30 p-3 min-w-0 [overflow-wrap:anywhere]">
            <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-white"><Send className="h-4 w-4 shrink-0 text-cyan-400" /> Telegram {d.telegramLinked ? <span className="text-[10px] uppercase text-emerald-300 ml-1">linked</span> : null}</div>
            {!d.telegramConfigured ? (
              ...
              <div className="mt-2 space-y-2">
                ...
                {code ? <div className="font-mono text-lg text-amber-200 tracking-widest break-all">{code}</div> : null}
                <div className="flex flex-wrap gap-1.5">
          ...
          <div className="rounded-lg border border-border bg-secondary/30 p-3 min-w-0 [overflow-wrap:anywhere]">
            <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-white"><Mail className="h-4 w-4 shrink-0 text-cyan-400" /> Email <span className="text-[11px] text-slate-500 font-normal break-all">{d.email}</span></div>
            {!d.emailConfigured ? (
