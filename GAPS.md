...
| API security (§11) | ROUND 4 + 5 | Secrets encrypted at rest (Round 4). **Round 5:** TOTP two-factor authentication (Settings → Two-Factor; enforced at sign-in), DB-backed rate limiting on sign-in (10/email, 30/IP per 15 min) and signup (5/IP, 60 global per hour). Still missing: token rotation. |
